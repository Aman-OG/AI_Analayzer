...
const analyzeWithGemini = async (resumeText, jobDescriptionText, mustHaveSkills = [], focusAreas = []) => {
  ...
  while (attempt < MAX_RETRIES) {
    try {
      ...
      logger.error(`Gemini prompt blocked`, {
        reason: blockReason,
        safetyRatings
      });
      ...
      if (startIndex === -1 || endIndex === -1 || endIndex < startIndex) {
        logger.error("Gemini response does not appear to contain valid JSON object markers", {
          jsonSnippet: cleanedJsonText.substring(0, 200)
        });
...
const triggerGeminiAnalysis = async (resumeId) => {
  ...
  try {
    ...
    logger.info(`Analysis completed successfully for resumeId: ${resumeId}`, {
      ...
      warningsCount: updatedAnalysisResult.warnings.length
    });
