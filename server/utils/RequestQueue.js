...
/**
 * Singleton queue for managing API requests with rate limiting
...
class RequestQueue extends EventEmitter {
    constructor() {
        ...
        console.log('✅ RequestQueue initialized (6s delay between requests)');
    }
    ...
    async processNext() {
        ...
        try {
            ...
            if (timeSinceLastRequest < this.minDelayMs) {
                ...
                console.log(`⏳ RequestQueue: Waiting ${waitTime}ms before next request`);
    ...
    async handleError(error, task) {
        ...
        if (errorMessage.includes('429') || errorMessage.includes('Resource has been exhausted')) {
            ...
            if (errorMessage.includes('GenerateRequestsPerDay')) {
                console.error('❌ Daily API quota exceeded - permanent error');
